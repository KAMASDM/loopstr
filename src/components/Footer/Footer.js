...
const socialLinks = [
  {
    name: "Facebook",
    icon: Facebook,
    href: "https://www.facebook.com/people/loopstr/",
  },
  { name: "Twitter", icon: Twitter, href: "https://x.com/loopstr" },
  {
    name: "Instagram",
    icon: Instagram,
    href: "https://www.instagram.com/loopstr/",
  },
];
...
const Footer = () => {
  ...
  return (
    <footer className="bg-green-100 text-gray-700 relative print:hidden">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div>
            <Link
              href="/"
              className="flex items-center space-x-2 text-2xl font-bold text-green-800"
              aria-label="Loopstr homepage"
            >
              <Recycle className="h-8 w-8 text-green-600" />
              <span>Loopstr</span>
            </Link>
            <p className="mt-4 text-gray-600">
              Empowering sustainable living through reducing waste, reusing
              resources, and responsible recycling for a greener planet.
            </p>
            <div
            ...
            >
              {socialLinks.map(({ name, icon: Icon, href }) => (
                <a
                  ...
                  aria-label={`Follow Loopstr on ${name}`}
                  role="listitem"
                >
        ...
        <div className="border-t border-green-200 pt-8 mt-8 text-center">
          <p className="text-gray-600 text-sm">
            © {currentYear} Loopstr. All rights reserved | Design & Developed By{" "}
            <a
