...
export default function TagBlogs({ slug }) {
  ...
  useEffect(() => {
    const fetchTagBlogs = async () => {
      if (!slug) return;
      try {
        const response = await ajaxCall("/all-posts/?site_domain=loopstr.co", {
          method: "GET",
        });
        ...
        if (filtered.length > 0) {
          ...
          if (matchedTag) {
            ...
            setTagDescription(
              `Explore our collection of articles about ${matchedTag.name}. Learn how ${matchedTag.name} can help you disconnect from digital distractions, improve mental clarity, and support a healthier, more mindful lifestyle through practical digital wellness strategies.`
            );
          }
        }
      } catch (error) {
                ...
                >
                  ...
                  <div className="p-4 sm:p-6 flex flex-col flex-grow">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-green-600 text-sm sm:text-base font-medium hover:text-green-700 transition-colors">
                        Read More →
                      </span>
                    </div>
                  </div>
